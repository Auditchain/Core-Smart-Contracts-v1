...
contract Members is Ownable, AccessControl {
    ...
    mapping(address => bool) enterpriseMap;
    ...
    // mapping (address => mapping(address => Invitation)) invitations1;
    // uint public invitationCount;
    ...
    function inviteValidator(address invitee, AuditTypes audit) public {
        ...
        require( isValidatorInvited(msg.sender, invitee, audit) == 0 , "Members:inviteValidator - This validator has been already invited for this validation type." );
        require( enterpriseMap[msg.sender], "Members:inviteValidator - Only Enterprise user can invite Validators.");
        require( validatorMap[invitee], "Members:inviteValidator - Only Approved Validators can be invited.");
        require( deposits[invitee] > 0,"Members:inviteValidator - This validator has not staked any tokens yet.");
    ...
    function acceptInvitation(address enterprise, uint256 invitationNumber) public {
        ...
        require( invitations[enterprise].length > invitationNumber, "Members:acceptInvitation - This invitation doesn't exist");
        require( invitations[enterprise][invitationNumber].acceptanceDate == 0, "Members:acceptInvitation- This invitation has been already accepted.");
        require( invitations[enterprise][invitationNumber].validator == msg.sender, "Members:acceptInvitation - You are accepting invitation to which you were not invited or this invitation doesn't exist.");
    ...
    function isValidatorInvited(address enterprise, address validator, AuditTypes types) internal view returns (uint256) {
    ...
    function createCohort(AuditTypes audit ) public {
        ...
        require(k >= 3, "Members:createCohort - Cohort requires at least 3 validators.");
        ...
        address cohortAddress;
          ...
          uint32 size;
            assembly {
                size := extcodesize(cohortAddress)
            }
         assembly {
        ...
        require(size == 0, "Members:createCohort - Cohort contract has been already created");
        Cohort(cohortAddress).initialize(auditToken, this,msg.sender, validatorsList, uint256(audit));
...
contract Cohort is Ownable, AccessControl {
    ...
    uint256 amountTokensPerDay = 683 * 10e18;
    Token private _auditToken;                         //AUDT token 
    Members private _membersContract;
    ...
    function redeem() public {
        ...
        uint256 amount;
        amount = _auditToken.balanceOf(msg.sender);
        deposits[msg.sender] = deposits[msg.sender].sub(amount);
    ...
    function updateDailyEarningsTransferFunds(address[] memory attesters, uint256[] memory tokens) public isController() {
        ...
        _auditToken.mint(address(this), amountTokensPerDay);
        emit RewardsDeposited(block.timestamp);
    }
    ...
    function initializeValidation(bytes32 documentHash) public {
        ...
        require(enterprise == msg.sender, "Cohort:initializeValidation - Only enterprise can call this function");
    ...
    function allocateRewardsPerValidation(bytes32 validationHash)public view isController() returns (address[] memory, uint256[] memory, ValidationStatus[] memory) {
        ...
        for (uint256 i=0; i< validators.length; i++ ){               
            stake[i] = _membersContract.deposits(validators[i]);
            validatorsValues[i] = validation.validatorChoice[validators[i]];
        }
        return (validatorsList, stake, validatorsValues);
    }

    function calculateVoteQuorum(bytes32 validationHash) internal view returns(uint256) {
        ...
        for (uint256 i=0; i< validators.length; i++ ){
            totalStaked += _membersContract.deposits(validators[i]);
            if (validation.validatorChoice[validators[i]] ==  ValidationStatus.Yes || validation.validatorChoice[validators[i]] == ValidationStatus.No )
                currentlyVoted += _membersContract.deposits(validators[i]);
        }
