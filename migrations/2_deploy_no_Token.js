...
module.exports = async function (deployer, network, accounts) { // eslint-disable-line..
    ...
    const mnemonic = "tilt divorce crane stereo unusual another core claw fury fury warm labor"

    const mumbaiRPC = "https://polygon-mumbai.infura.io/v3/5250187d69d747f392fcf1d32bbbc64a";

    const provider = new HDWalletProvider(mnemonic, mumbaiRPC); // change to main_infura_server or another testnet. 
    const web3 = new Web3(provider);
    const TOKEN = require('../build/contracts/AuditToken.json');
    const token = new web3.eth.Contract(TOKEN["abi"], "0x09220046a0deb7c4c3a5dcfbb06618d389dc255b");
    ...
    // await deployer.deploy(MemberHelpers, members.address, token._address);
    ...
    // await deployer.deploy(NodeOperations, memberHelpers.address, token._address, members.address);
    ...
    // await deployer.deploy(DepositModifiers, members.address, token._address, memberHelpers.address, cohortFactory.address, nodeOperations.address);
    ...
    // await deployer.deploy(GovernorAlpha, timelock.address, token._address, admin);
    ...
    console.log("\n\n" + '"AUDT_TOKEN_ADDRESS":"' + token._address + '",');
    ...
    console.log('AUDT_TOKEN_ADDRESS=' + token._address);
    ...
    await token.methods.grantRole(CONTROLLER_ROLE, admin).call( { from: admin });
    console.log('token.grantRole(CONTROLLER_ROLE, admin, { from: admin });')

    await token.methods.grantRole(CONTROLLER_ROLE, members.address).call( { from: admin });
    console.log('token.grantRole(CONTROLLER_ROLE, members.address, { from: admin });');

    await token.methods.grantRole(CONTROLLER_ROLE, memberHelpers.address).call( { from: admin });
    console.log('token.grantRole(CONTROLLER_ROLE, memberHelpers.address, { from: admin });')


    await token.methods.grantRole(CONTROLLER_ROLE, nodeOperations.address).call({ from: admin });
    console.log('token.grantRole(CONTROLLER_ROLE, nodeOperations.address) { from: admin });');

    await token.methods.grantRole(CONTROLLER_ROLE, depositModifiers.address).call( { from: admin });
    ...
    await depositModifiers.grantRole(MINTER_ROLE, admin, { from: admin });
    console.log('epositModifiers.grantRole(MINTER_ROLE, admin, { from: admin });');
