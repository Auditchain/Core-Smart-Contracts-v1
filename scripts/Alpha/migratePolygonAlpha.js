...
async function getBurnInfo(transactionHash) {
    ...
    try {
        ...
        const result = await web3Polygon.eth.getTransactionReceipt(transactionHash);
        console.log("from", result.from);
        console.log("data", result.logs[0].data);
        return [Number(result.logs[0].data), result.from];
        // return ["37392186600244070000", "0xa527c8eaa15ede09bd81300b5980d702e3fdfe79"];
    }
    catch (err) {
        console.log(err);
        readTxCount++;
...
async function hanbdleRegularPurchasers(burnedTokenInfo) {
    ...
    try {
        ...
        console.log("Executing Regular Purchasers");
        const vestInfo = await salePolygon.methods.tokenHolders(burnedTokenInfo[1]).call();

        console.log("Record from regular purchasers:", vestInfo);
...
async function hanbdleRegularPurchasersOld(burnedTokenInfo) {

    try {

        console.log("Executing Regular Purchasers Old");
        const vestInfo = await salePolygonOld.methods.tokenHolders(burnedTokenInfo[1]).call();

        console.log("Record from regular purchasers old:", vestInfo);
...
async function hanbdleBridgeUsers(burnedTokenInfo) {

    try {

        console.log("Executing Bridge Users");
...
async function handleEarlyInvestors(burnedTokenInfo) {
    ...
    try {
        ...
        console.log("Executing Early Investors");
...
async function sendNotification(txData, status, tx) {
    ...
    try {
        ...
        let info = await transporter.sendMail({
            ...
            // to: "bogdanfiedur@gmail.com, win2win@protonmail.com", // list of receivers
            to: "bogdanfiedur@gmail.com, support@auditchain.com", // list of receivers
            subject: "migration invoked Polygon for " + tx, // Subject line
...
app.get('/get_transaction', function (req, res) {
    ...
    getBurnInfo(req.query.tx).then(async function (burnedTokenInfo) {
        ...
        if (result == "EMPTY")
            result = await hanbdleRegularPurchasersOld(burnedTokenInfo);

        if (result == "EMPTY")
            result = await handleEarlyInvestors(burnedTokenInfo);

        if (result == "EMPTY")
            result = await hanbdleBridgeUsers(burnedTokenInfo);
...
var server = app.listen(8187, function () {
